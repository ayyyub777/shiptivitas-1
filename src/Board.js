...
export default class Board extends React.Component {
  ...
  componentDidMount() {
    this.drake = Dragula([
      ...
      // Current color
      const elColor = el.className.split(" ").find((c) => c.includes("Card-"));
      // Sibling color
      const siblingColor = sibling.childNodes[0].className
        .split(" ")
        .find((c) => c.includes("Card-"));
      // Update current color based on sibling color
      el.className = el.className.replace(elColor, siblingColor);
    });
  }
